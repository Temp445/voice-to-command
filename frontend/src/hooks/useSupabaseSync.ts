...
export function useSupabaseSync() {
  ...
  useEffect(() => {
    ...
    // Fetch settings on initial load or user change
    const fetchSettings = async () => {
      // Defensive wait for local token to be in place (backend sync)
      let token = null;
      for (let i = 0; i < 20; i++) {
        token = localStorage.getItem("ace-local-token");
        if (token) break;
      ...
      try {
        ...
        console.log('[Supabase Realtime] Initial settings successfully synced.');
      } catch (e) {
        console.error('[Supabase Realtime] Failed to fetch initial settings:', e);
      }
    };
